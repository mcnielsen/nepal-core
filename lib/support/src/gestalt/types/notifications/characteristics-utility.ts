...
export class CharacteristicsUtility {
    ...
    public static getIntegrationsDefinition(): AlCardstackPropertyDescriptor {
        return {
            ...
            caption: AlLocatorService.getCurrentEnvironment() === 'integration' ? 'Subscribed Templated Connection' : 'Subscribed Connector',
    ...
    public static async validateEntitlement(accountId:string, entitlement:string):Promise<boolean>{
        let entitlements:AlEntitlementCollection = await SubscriptionsClient.getEntitlements(accountId);
