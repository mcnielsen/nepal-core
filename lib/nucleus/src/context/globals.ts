import { AlSessionInstance } from '../session';
import { AlLocatorMatrix } from '../common/types';
import { RootClient } from '../client/root-client';

/**
 * These three globals are provided for compatibility with previous architecture.  Please do not use them directly;
 * instead, use the `session`, `locator`, and `rootClient` properties on `AlExecutionContext`.
 */

/* tslint:disable:variable-name */
export let AlSession:AlSessionInstance;
export let AlLocatorService:AlLocatorMatrix;
export let AlDefaultClient:RootClient;

export function initializeAlGlobals( session:AlSessionInstance, locator:AlLocatorMatrix, rootClient:RootClient ) {
    AlSession = AlSession || session;
    AlLocatorService = AlLocatorService || locator;
    AlDefaultClient = AlDefaultClient || rootClient;
}
 ...
 * and for ensuring that singleton instances are, in fact, singletons.  It is deprecated, so please don't use
 * it directly.
 */
